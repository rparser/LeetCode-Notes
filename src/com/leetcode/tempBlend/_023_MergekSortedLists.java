...
public class _023_MergekSortedLists {
    public ListNode stringToListNode(String sortedString) {
        String[] strs = sortedString.split(" ");
        if (strs.length < 1) return null;
        ListNode node = new ListNode(Integer.valueOf(strs[0]));
        ListNode head = node;
        for (int i = 1; i < strs.length; i++) {
            ListNode tmp = new ListNode(Integer.valueOf(strs[i]));
            node.next = tmp;
            node = node.next;
        }
        return head;
